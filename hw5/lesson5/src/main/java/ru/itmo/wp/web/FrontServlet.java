...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response) throws NotFoundException, ServletException, IOException {
        ...
        Map<Class<?>, Object> defaultParameters = Map.of(HttpServletRequest.class, request, Map.class, view);
        try {
            List<Object> arguments = new ArrayList<>();
            for (Class<?> parameterType : parameterTypes) {
                Object argument = defaultParameters.get(parameterType);
                if (argument != null) {
                    arguments.add(argument);
                } else {
                    throw new ServletException("Invalid method signature [method=" + method.getName() + "]");
                }
            }
        ...
        if (lang != null && !lang.equals("en")) {
            simpleNames.add(pageClass.getSimpleName() + "_" + lang + ".ftlh");
        }
        simpleNames.add(pageClass.getSimpleName() + ".ftlh");

        Template template = newTemplate(simpleNames);
    ...
    private Template newTemplate(List<String> simpleNames) throws ServletException {
        Template template = null;
        for (String templateName : simpleNames) {
        ...
        if (template == null) {
            throw new ServletException("Can't find template [templateName=" + simpleNames.get(simpleNames.size() - 1) + "]");
        }
